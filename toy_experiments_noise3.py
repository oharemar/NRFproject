...
if __name__ == '__main__':
    ...
    for sample in samples:
        ...
        for noise in noises:
            # random_state = 1
            X, y = dt.make_blobs(sample, 2, centers=[[2, 3], [1, 5], [3, 5]], center_box=(0, 7), cluster_std=0.5+noise,random_state=10)

            # X_2, y_2 = datasets.make_classification(sample, 2, 2, 0, n_classes=3, n_clusters_per_class=1, random_state=1)

            # X_3, y_3 = datasets.make_gaussian_quantiles(n_samples=sample, n_classes=3, random_state=5)

            #X, y = datasets.make_circles(sample, noise=noise, random_state=3)

            #X, y = datasets.make_moons(sample, noise=noise, random_state=2)
            ...
            for model in names:
                ...
                try:
                    if model == 'Input data':
                        ax.set_ylabel('Noise = {}'.format(noise))
                        if i == 1:
                            ax.set_title(model)
                        ...
                        mod.compile(loss='categorical_crossentropy',
                                    optimizer=sgd,  # 'sgd',
                                    metrics=['accuracy'])
                        mod.fit(X_train, y_train_keras, epochs=25)
                except:
        ...
        i = 1
        plt.tight_layout()
        fig.savefig('noise3{}_samples{}'.format(dataset, sample))
        dataset += 1
