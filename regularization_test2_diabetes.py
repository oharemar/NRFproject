...
if __name__ == '__main__':
    ...
    learn_rates_nrfdw = list(np.linspace(0, 10, num=15))
    learn_rates_nrf = list(np.linspace(0, 10, num=15))
    learn_rates_nrfel = list(np.linspace(0, 10, num=15))
    learn_rates_nrfeldw = list(np.linspace(0, 10, num=15))
        ...
        for train_index, test_index in kf.split(df):
            ...
            for eta, eta_nrfdw, eta_nrfel, eta_nrfeldw in zip(learn_rates_nrf, learn_rates_nrfdw, learn_rates_nrfel,
                ...
                nrfdw = NeuralRandomForest(rf, 'NRF_analyticWeights_adam', X_train, y_train, output_func='softmax',
                                           ...
                                           gamma_output=1, gamma=[5, 5])  # zde změna, gamma_output je 1
                ...
                nrf_eldw_ultra = NeuralRandomForest(rf, 'NRF_extraLayer_analyticWeights_ultra_adam', X_train, y_train,
                                              ...
                                              cost_func='CrossEntropy',
                                              gamma_output=1, gamma=[5, 5])  # zde změna, gamma_output je 1
                nrf_eldw_ultra.get_NRF_ensemble(30, 10, 0.0045, eta)
    ...
    fig.savefig('NRF_regularization_acc_bigBeta_diabetes.png')
    ...
    fig.savefig('NRF_regularization_F1_bigBeta_diabetes.png')
    ...
    for j in range(accuracy_nrfdw_temp.shape[0]):
        ax.errorbar(learn_rates_nrf, accuracy_nrfdw_temp[j,:], yerr=None, marker='o',color = 'lightgray',alpha = 0.5,zorder=-32)
    ax.errorbar(learn_rates_nrf, accuracy_nrfdw,yerr = accuracy_nrfdw_std,ecolor='darkorange',marker = 'o')
    plt.title('NRF_DW')
    plt.ylim([np.amin(accuracy_nrfdw_temp) - 0.05, np.amax(accuracy_nrfdw_temp) + 0.05])
    fig.savefig('NRF_DW_regularization_acc_bigBeta_diabetes.png')
    ...
    for j in range(f_nrfdw_temp.shape[0]):
        ax.errorbar(learn_rates_nrf, f_nrfdw_temp[j,:], yerr=None, marker='o',color = 'lightgray',alpha = 0.5,zorder=-32)
    ax.errorbar(learn_rates_nrf, f_nrfdw,yerr = f_nrfdw_std,ecolor='darkorange',marker = 'o')
    plt.title('NRF_DW')
    plt.ylim([np.amin(f_nrfdw_temp) - 0.05, np.amax(f_nrfdw_temp) + 0.05])
    fig.savefig('NRF_DW_regularization_F1_bigBeta_diabetes.png')
    ...
    for j in range(accuracy_nrfeldw_temp.shape[0]):
        ax.errorbar(learn_rates_nrf, accuracy_nrfeldw_temp[j,:], yerr=None, marker='o',color = 'lightgray',alpha = 0.5,zorder=-32)
    ax.errorbar(learn_rates_nrf, accuracy_nrfeldw,yerr = accuracy_nrfeldw_std,ecolor='darkorange',marker = 'o')
    plt.title('NRF_EL_DW')
    plt.ylim([np.amin(accuracy_nrfeldw_temp) - 0.05, np.amax(accuracy_nrfeldw_temp) + 0.05])
    fig.savefig('NRF_EL_DW_regularization_acc_bigBeta_diabetes.png')
    ...
    for j in range(f_nrfeldw_temp.shape[0]):
        ax.errorbar(learn_rates_nrf, f_nrfeldw_temp[j,:], yerr=None, marker='o',color = 'lightgray',alpha = 0.5,zorder=-32)
    ax.errorbar(learn_rates_nrf, f_nrfeldw,yerr = f_nrfeldw_std,ecolor='darkorange',marker = 'o')
    plt.title('NRF_EL_DW')
    plt.ylim([np.amin(f_nrfeldw_temp) - 0.05, np.amax(f_nrfeldw_temp) + 0.05])
    fig.savefig('NRF_EL_DW_regularization_F1_bigBeta_diabetes.png')
    ...
    for j in range(accuracy_nrfeldw_ultra_temp.shape[0]):
        ax.errorbar(learn_rates_nrf, accuracy_nrfeldw_ultra_temp[j, :], yerr=None, marker='o', color='lightgray', alpha=0.5,
                    zorder=-32)
    ax.errorbar(learn_rates_nrf, accuracy_nrfeldw_ultra, yerr=accuracy_nrfeldw_ultra_std, ecolor='darkorange', marker='o')
    plt.title('NRF_EL_DW_identity')
    plt.ylim([np.amin(accuracy_nrfeldw_ultra_temp) - 0.05, np.amax(accuracy_nrfeldw_ultra_temp) + 0.05])
    fig.savefig('NRF_EL_DW_identity_regularization_acc_bigBeta_diabetes.png')
    ...
    for j in range(f_nrfeldw_ultra_temp.shape[0]):
        ax.errorbar(learn_rates_nrf, f_nrfeldw_ultra_temp[j, :], yerr=None, marker='o', color='lightgray', alpha=0.5,
                    zorder=-32)
    ax.errorbar(learn_rates_nrf, f_nrfeldw_ultra, yerr=f_nrfeldw_ultra_std, ecolor='darkorange', marker='o')
    plt.title('NRF_EL_DW_identity')
    plt.ylim([np.amin(f_nrfeldw_ultra_temp) - 0.05, np.amax(f_nrfeldw_ultra_temp) + 0.05])
    fig.savefig('NRF_EL_DW_identity_regularization_F1_bigBeta_diabetes.png')