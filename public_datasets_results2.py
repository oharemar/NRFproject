...
datasets = ['valley','wine'] # pak přidáme USPS a drive diagnosis


# do modelů pak přidat logistic regression, RF a neuronku z kerasu
model_names = ['NRF_analyticWeights','NRF_analyticWeights_adam','NRF_analyticWeights_nesterov','NRF_basic',
               'NRF_basic_adam','NRF_basic_nesterov','NRF_extraLayer','NRF_extraLayer_adam','NRF_extraLayer_nesterov',
...
for dataset in datasets:
    ...
    for model,eta in zip(model_names,learn_rates):
        ...
        with open('RESULTS_PUBLIC_DATASETS2/{}/{}_mean.txt'.format(model,dataset), 'w') as file:
            file.write(json.dumps(final_results_vals))
        with open('RESULTS_PUBLIC_DATASETS2/{}/{}_std.txt'.format(model, dataset), 'w') as file:
