...
class Network():
    ...
    def __init__(self , sizes, biases = None, weights = None , gamma=None,gamma_output = 2,
                 weight_initilizer = 'new', cost = None, output_func = 'sigmoid',gamma_sigmoid = 1,penultimate_func = 'LeakyReLU',
                 alpha = 0.01,momentum1 = 0.9, momentum2 = 0.99,epsilon = 1e-8):
        ...
        self.alpha = alpha # alpha u leaky relu
        ...
        self.m_weights = []  # nastavíme na 0
        self.n_weights = []  # nastavíme na 0
        self.m_biases = []  # nastavíme na 0
        self.n_biases = []  # nastavíme na 0
        for w in self.weights:
            self.m_weights.append(np.zeros(w.shape))
            self.n_weights.append(np.zeros(w.shape))
        for b in self.biases:
            self.m_biases.append(np.zeros(b.shape))
            self.n_biases.append(np.zeros(b.shape))
    ...
    def update_mini_batch(self, mini_batch, eta, lmbda, n):
        ...
        self.m_biases = [(self.momentum1 * m + (1 - self.momentum1) * w) for m, w in zip(self.m_biases, nabla_b)]
        self.n_weights = [(self.momentum2 * n + (1 - self.momentum2) * (w ** 2)) for n, w in
                          zip(self.n_weights, nabla_w)]
        self.n_biases = [(self.momentum2 * n + (1 - self.momentum2) * (w ** 2)) for n, w in zip(self.n_biases, nabla_b)]

        weights_update = [(1 / (1 - self.momentum1 ** self.t)) * m / (
                    ((1 / (1 - self.momentum2 ** self.t)) * n) ** 0.5 + self.epsilon) for m, n in
                          zip(self.m_weights, self.n_weights)]
        biases_update = [(1 / (1 - self.momentum1 ** self.t)) * m / (
                    ((1 / (1 - self.momentum2 ** self.t)) * n) ** 0.5 + self.epsilon) for m, n in
