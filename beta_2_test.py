...
"""TEST PRO BETA2 parametr v první skryté vrstvě, BETA1 bude zatím nastaven na 1"""
...
if __name__ == '__main__':
    ...
    for train_index, test_index in kf.split(df):
        ...
        for eta, eta_nrfdw, eta_nrfel, eta_nrfeldw in zip(learn_rates_nrf, learn_rates_nrfdw, learn_rates_nrfel,
            ...
            nrf = NeuralRandomForest(rf, 'NRF_basic_adam', X_train, y_train, output_func='softmax',
                                     cost_func='CrossEntropy',
                                     gamma_output=1.5, gamma=[1, eta])  # zde změna, gamma_output je 1
            ...
            nrf_el = NeuralRandomForest(rf, 'NRF_extraLayer_adam', X_train, y_train, output_func='softmax',
                                        cost_func='CrossEntropy',
                                        gamma_output=1, gamma=[1, eta])  # zde změna, gamma_output je 1
    ...
    fig.savefig('NRF_DW_beta2_acc.png')
    ...
    fig.savefig('NRF_DW_beta2_F1.png')
    ...
    plt.xlabel(r'$\beta_2$')
    plt.ylabel('Accuracy')
    plt.xlim([min(learn_rates_nrfel) - (learn_rates_nrfel[1] - learn_rates_nrfel[0]),
              max(learn_rates_nrfel) + (learn_rates_nrfel[1] - learn_rates_nrfel[0])])
    plt.title('NRF_EL')
    plt.ylim([min(accuracy_nrfel) - 0.05, max(accuracy_nrfel) + 0.05])
    fig.savefig('NRF_EL_beta2_acc.png')
    ...
    fig.savefig('NRF_EL_beta2_F1.png')
    ...
    fig.savefig('NRF_EL_DW_beta2_acc.png')
    ...
    fig.savefig('NRF_EL_DW_beta2_F1.png')
