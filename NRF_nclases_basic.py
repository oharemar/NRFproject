...
class NeuralTreeBasic():
    ...
    def __init__(self, decision_tree = None, class_probs = None):
        """here we need to add one attribute that stores class probabilities of DT in individual leaves"""
        self.decision_tree = decision_tree
        self.class_probs = class_probs # this is list of probabilities (normalized by ratio of one class in specific leaf) of classes in individual leaves
    ...
    def initialize_third_hidden_layer(self):
        """in this case the weights will be arranged in same manner as the previous ones, but
        backpropagation needs to be modified a little"""
        third_hidden_layer_weights = np.zeros((self.decision_tree.n_classes_*len(self.leaves),len(self.leaves)),dtype=np.float64)
        for leaf,leaf_number in zip(self.leaves,range(len(self.leaves))):
            leaf_part_weights = self.class_probs[leaf_number] # weights are normalized probabilities in corresponding leaves
            leaf_part_weights = np.array(leaf_part_weights,dtype=np.float64)
            third_hidden_layer_weights[leaf_number*self.decision_tree.n_classes_:(leaf_number+1)*self.decision_tree.n_classes_,leaf_number] = leaf_part_weights


        third_hidden_layer_biases = np.zeros((self.decision_tree.n_classes_*len(self.leaves),1),dtype=np.float64)

        self.weights.append(third_hidden_layer_weights)
        self.biases.append(third_hidden_layer_biases)


    def initialize_output_layer(self):
        weights = np.zeros((self.decision_tree.n_classes_, len(self.leaves)*self.decision_tree.n_classes_),dtype=np.float64)
        for label in range(self.decision_tree.n_classes_):
            leaf_part_weights = [0 for j in range(len(self.leaves)*self.decision_tree.n_classes_)]
            leaf_part_weights[label:len(self.leaves)*self.decision_tree.n_classes_:self.decision_tree.n_classes_] = [1 for j in range(len(self.leaves))]
            leaf_part_weights = np.array(leaf_part_weights,dtype=np.float64)
