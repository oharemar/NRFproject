...
if __name__ == '__main__':
    ...
    for sample in samples:
        ...
        datasets = [(X_4,y_4)]
        names = ['Input data','Random Forest', 'NRF_DW', 'NRF_EL_DW_id']
        ...
        for X,y in datasets:
            ...
            rf = RandomForestClassifier(n_estimators=2, max_depth=20, random_state=1, max_features=1.0) # overfitting
            ...
            for term in regularization:
                for model in names:
                    ax = plt.subplot(len(regularization), len(names), i)
                    ax.set_xticks(())
                    ax.set_yticks(())
                    if model == 'Input data':
                        ax.set_ylabel('Reg.constant = {}'.format(term))
                        if i == 1:
                            ax.set_title(model)
                        ...
                        mod.compile(loss='categorical_crossentropy',
                                    optimizer=sgd,  # 'sgd',
                                    metrics=['accuracy'])
                        mod.fit(X_train, y_train_keras, epochs=term)
                    if model == 'NN':
            ...
            i = 1
            plt.tight_layout()
            fig.savefig('reguarization_{}_samples{}'.format(dataset,sample))
            dataset += 1
