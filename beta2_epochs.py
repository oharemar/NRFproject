...
if __name__ == '__main__':
    ...
    accuracy_nrfdw = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)
    accuracy_nrf = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)
    accuracy_nrfeldw = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)
    accuracy_nrfeldw_ultra = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)
    accuracy_rf = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)


    f1_nrfdw = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)
    f1_nrf = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)
    f1_nrfeldw = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)
    f1_nrfeldw_ultra = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)
    f1_rf = np.zeros((len(beta2), len(epochs), n), dtype=np.float64)
            ...
            for beta2_number, j in zip(beta2, range(len(beta2))):
                for epoch,k in zip(epochs,range(len(epochs))):
                    ...
                    nrfdw = NeuralRandomForest(rf, 'NRF_analyticWeights_adam', X_train, y_train, output_func='softmax',
                                               ...
                                               gamma_output=1, gamma=[1, beta2_number])
                    ...
                    nrf = NeuralRandomForest(rf, 'NRF_basic_adam', X_train, y_train, output_func='softmax',
                                             cost_func='CrossEntropy',
                                             gamma_output=1, gamma=[1, beta2_number])  # zde změna, gamma_output je 1
    ...
    fig.savefig('NRF_epochs_beta2_acc.png')
    ...
    for k in range(len(epochs)):
        for j in range(len(beta2)):
            text = ax.text(k, j, u'{:.3f}\n\u00B1\n{:.3f}'.format(f1_nrf_mean[j, k], f1_nrf_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_epochs_beta2_f1.png')
    ...
    for k in range(len(epochs)):
        for j in range(len(beta2)):
            text = ax.text(k, j, u'{:.3f}\n\u00B1\n{:.3f}'.format(accuracy_nrfdw_mean[j, k], accuracy_nrfdw_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_DW_epochs_beta2_acc.png')
    ...
    for k in range(len(epochs)):
        for j in range(len(beta2)):
            text = ax.text(k, j, u'{:.3f}\n\u00B1\n{:.3f}'.format(f1_nrfdw_mean[j, k], f1_nrfdw_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_DW_epochs_beta2_f1.png')
    ...
    for k in range(len(epochs)):
        for j in range(len(beta2)):
            text = ax.text(k, j, u'{:.3f}\n\u00B1\n{:.3f}'.format(accuracy_nrfeldw_mean[j, k], accuracy_nrfeldw_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_EL_DW_epochs_beta2_acc.png')
    ...
    for k in range(len(epochs)):
        for j in range(len(beta2)):
            text = ax.text(k, j, u'{:.3f}\n\u00B1\n{:.3f}'.format(f1_nrfeldw_mean[j, k], f1_nrfeldw_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_EL_DW_epochs_beta2_f1.png')
    ...
    for k in range(len(epochs)):
        for j in range(len(beta2)):
            text = ax.text(k, j,
                           ...
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_EL_DW_identity_epochs_beta2_acc.png')
    ...
    for k in range(len(epochs)):
        for j in range(len(beta2)):
            text = ax.text(k, j, u'{:.3f}\n\u00B1\n{:.3f}'.format(f1_nrfeldw_ultra_mean[j, k], f1_nrfeldw_ultra_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_EL_DW_identity_epochs_beta2_f1.png')
        ...
        for j in range(len(beta2)):
            text = ax.text(k, j,
                           u'{:.3f}\n\u00B1\n{:.3f}'.format(accuracy_rf_mean[j, k],
                                                            accuracy_rf_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('RF_epochs_beta2_acc.png')
    ...
    for k in range(len(epochs)):
        for j in range(len(beta2)):
            text = ax.text(k, j,
                           ...
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('RF_epochs_beta2_f1.png')
