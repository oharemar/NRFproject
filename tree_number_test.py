...
"""budeme měnit počet STROMŮ a sledovat vývoj accuracy a loss (cross entropy)"""
...
if __name__ == '__main__':
    ...
    epochs = [5,10,20,30,40,50,60,70,80] # number of trees in random forest, we leave "epochs" in order to avoid renaming in entire script
    ...
    for number in epochs:
        ...
        loss_temp_train_rf = []
        loss_temp_test_rf = []
        accuracy_temp_train_rf = []
        accuracy_temp_test_rf = []




        for train_index, test_index in kf.split(df):
            rf = RandomForestClassifier(n_estimators=number, criterion='entropy', max_depth=6, max_features='auto')
            ...
            predictions_test_loss_rf= rf.predict_proba(X_test)
            predictions_train_rf = rf.predict(X_train)
            predictions_train_loss_rf = rf.predict_proba(X_train)

            results_test_rf = classification_report(y_test, predictions_test_rf, output_dict=True)
            results_train_rf = classification_report(y_train, predictions_train_rf, output_dict=True)

            accuracy_temp_test_rf.append(results_test_rf['accuracy'])
            accuracy_temp_train_rf.append(results_train_rf['accuracy'])

            loss_temp_test_rf.append(log_loss(y_test, predictions_test_loss_rf))
            loss_temp_train_rf.append(log_loss(y_train, predictions_train_loss_rf))
            ...
            nrf_el.get_NRF_ensemble(30, 10, 0.01, 0.02)
        ...
        accuracy_train_rf.append(accuracy_temp_train_rf)
        accuracy_test_rf.append(accuracy_temp_test_rf)
        loss_train_rf.append(loss_temp_train_rf)
        loss_test_rf.append(loss_temp_test_rf)
    ...
    accuracy_train_rf = list(np.mean(np.array(accuracy_train_rf, dtype=np.float64), axis=1))
    accuracy_test_rf = list(np.mean(np.array(accuracy_test_rf, dtype=np.float64), axis=1))
    loss_train_rf = list(np.mean(np.array(loss_train_rf, dtype=np.float64), axis=1))
    loss_test_rf = list(np.mean(np.array(loss_test_rf, dtype=np.float64), axis=1))

    accuracy_train_nrfdw = list(np.mean(np.array(accuracy_train_nrfdw, dtype=np.float64), axis=1))
    accuracy_test_nrfdw = list(np.mean(np.array(accuracy_test_nrfdw, dtype=np.float64), axis=1))
    loss_train_nrfdw = list(np.mean(np.array(loss_train_nrfdw, dtype=np.float64), axis=1))
    loss_test_nrfdw = list(np.mean(np.array(loss_test_nrfdw, dtype=np.float64), axis=1))

    accuracy_train_nrf = list(np.mean(np.array(accuracy_train_nrf, dtype=np.float64), axis=1))
    accuracy_test_nrf = list(np.mean(np.array(accuracy_test_nrf, dtype=np.float64), axis=1))
    loss_train_nrf = list(np.mean(np.array(loss_train_nrf, dtype=np.float64), axis=1))
    loss_test_nrf = list(np.mean(np.array(loss_test_nrf, dtype=np.float64), axis=1))

    accuracy_train_nrfel = list(np.mean(np.array(accuracy_train_nrfel, dtype=np.float64), axis=1))
    accuracy_test_nrfel = list(np.mean(np.array(accuracy_test_nrfel, dtype=np.float64), axis=1))
    loss_train_nrfel = list(np.mean(np.array(loss_train_nrfel, dtype=np.float64), axis=1))
    loss_test_nrfel = list(np.mean(np.array(loss_test_nrfel, dtype=np.float64), axis=1))

    accuracy_train_nrfeldw = list(np.mean(np.array(accuracy_train_nrfeldw, dtype=np.float64), axis=1))
    accuracy_test_nrfeldw = list(np.mean(np.array(accuracy_test_nrfeldw, dtype=np.float64), axis=1))
    loss_train_nrfeldw = list(np.mean(np.array(loss_train_nrfeldw, dtype=np.float64), axis=1))
    loss_test_nrfeldw = list(np.mean(np.array(loss_test_nrfeldw, dtype=np.float64), axis=1))


    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs,accuracy_train_nrf,color = 'red',marker='o')
    ax.plot(epochs, accuracy_train_nrf,color = 'red', label = 'NRF')
    ax.scatter(epochs, accuracy_train_rf, color='blue', marker='o')
    ax.plot(epochs, accuracy_train_rf, color='blue',label='RF')
    ...
    fig.savefig('NRF_trees_acc_train.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, accuracy_test_nrf, color='red', marker='o')
    ax.plot(epochs, accuracy_test_nrf, color='red',label='NRF')
    ax.scatter(epochs, accuracy_test_rf, color='blue', marker='o')
    ax.plot(epochs, accuracy_test_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    plt.ylabel('Accuracy')
    plt.xlim([0, 85])
    ...
    fig.savefig('NRF_trees_acc_test.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, loss_train_nrf, color='red', marker='o')
    ax.plot(epochs, loss_train_nrf, color='red',label='NRF')
    ax.scatter(epochs, loss_train_rf, color='blue', marker='o')
    ax.plot(epochs, loss_train_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    plt.ylabel('Loss')
    plt.xlim([0, 85])
    ...
    fig.savefig('NRF_trees_loss_train.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, loss_test_nrf, color='red', marker='o')
    ax.plot(epochs, loss_test_nrf, color='red',label='NRF')
    ax.scatter(epochs, loss_test_rf, color='blue', marker='o')
    ax.plot(epochs, loss_test_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    plt.ylabel('Loss')
    plt.xlim([0, 85])
    ...
    fig.savefig('NRF_trees_loss_test.png')


    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs,accuracy_train_nrfdw,color = 'red',marker='o')
    ax.plot(epochs, accuracy_train_nrfdw,color = 'red',label='NRF_DW')
    ax.scatter(epochs, accuracy_train_rf, color='blue', marker='o')
    ax.plot(epochs, accuracy_train_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_DW_trees_acc_train.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, accuracy_test_nrfdw, color='red', marker='o')
    ax.plot(epochs, accuracy_test_nrfdw, color='red',label='NRF_DW')
    ax.scatter(epochs, accuracy_test_rf, color='blue', marker='o')
    ax.plot(epochs, accuracy_test_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_DW_trees_acc_test.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, loss_train_nrfdw, color='red', marker='o')
    ax.plot(epochs, loss_train_nrfdw, color='red',label='NRF_DW')
    ax.scatter(epochs, loss_train_rf, color='blue', marker='o')
    ax.plot(epochs, loss_train_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_DW_trees_loss_train.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, loss_test_nrfdw, color='red', marker='o')
    ax.plot(epochs, loss_test_nrfdw, color='red',label='NRF_DW')
    ax.scatter(epochs, loss_test_rf, color='blue', marker='o')
    ax.plot(epochs, loss_test_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_DW_trees_loss_test.png')


    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs,accuracy_train_nrfel,color = 'red',marker='o')
    ax.plot(epochs, accuracy_train_nrfel,color = 'red',label='NRF_EL')
    ax.scatter(epochs, accuracy_train_rf, color='blue', marker='o')
    ax.plot(epochs, accuracy_train_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_EL_trees_acc_train.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, accuracy_test_nrfel, color='red', marker='o')
    ax.plot(epochs, accuracy_test_nrfel, color='red',label = 'NRF_EL')
    ax.scatter(epochs, accuracy_test_rf, color='blue', marker='o')
    ax.plot(epochs, accuracy_test_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_EL_trees_acc_test.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, loss_train_nrfel, color='red', marker='o')
    ax.plot(epochs, loss_train_nrfel, color='red',label='NRF_EL')
    ax.scatter(epochs, loss_train_rf, color='blue', marker='o')
    ax.plot(epochs, loss_train_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_EL_trees_loss_train.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, loss_test_nrfel, color='red', marker='o')
    ax.plot(epochs, loss_test_nrfel, color='red',label='NRF_EL')
    ax.scatter(epochs, loss_test_rf, color='blue', marker='o')
    ax.plot(epochs, loss_test_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_EL_trees_loss_test.png')


    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs,accuracy_train_nrfeldw,color = 'red',marker='o')
    ax.plot(epochs, accuracy_train_nrfeldw,color = 'red',label='NRF_EL_DW')
    ax.scatter(epochs, accuracy_train_rf, color='blue', marker='o')
    ax.plot(epochs, accuracy_train_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_EL_DW_trees_acc_train.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, accuracy_test_nrfeldw, color='red', marker='o')
    ax.plot(epochs, accuracy_test_nrfeldw, color='red',label='NRF_EL_DW')
    ax.scatter(epochs, accuracy_test_rf, color='blue', marker='o')
    ax.plot(epochs, accuracy_test_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_EL_DW_trees_acc_test.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, loss_train_nrfeldw, color='red', marker='o')
    ax.plot(epochs, loss_train_nrfeldw, color='red',label='NRF_EL_DW')
    ax.scatter(epochs, loss_train_rf, color='blue', marker='o')
    ax.plot(epochs, loss_train_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_EL_DW_trees_loss_train.png')

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(epochs, loss_test_nrfeldw, color='red', marker='o')
    ax.plot(epochs, loss_test_nrfeldw, color='red',label='NRF_EL_DW')
    ax.scatter(epochs, loss_test_rf, color='blue', marker='o')
    ax.plot(epochs, loss_test_rf, color='blue', label='RF')
    plt.legend(loc="lower right")
    plt.xlabel('Trees')
    ...
    fig.savefig('NRF_EL_DW_trees_loss_test.png')
