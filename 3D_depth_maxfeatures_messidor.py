...
if __name__ == '__main__':
    ...
    accuracy_nrfdw = np.zeros((len(max_features), len(depth), n), dtype=np.float64)
    accuracy_nrf = np.zeros((len(max_features), len(depth), n), dtype=np.float64)
    accuracy_nrfeldw = np.zeros((len(max_features), len(depth), n), dtype=np.float64)
    accuracy_nrfeldw_ultra = np.zeros((len(max_features), len(depth), n), dtype=np.float64)
    accuracy_rf = np.zeros((len(max_features), len(depth), n), dtype=np.float64)


    f1_nrfdw = np.zeros((len(max_features), len(depth), n), dtype=np.float64)
    f1_nrf = np.zeros((len(max_features), len(depth), n), dtype=np.float64)
    f1_nrfeldw = np.zeros((len(max_features), len(depth), n), dtype=np.float64)
    f1_nrfeldw_ultra = np.zeros((len(max_features), len(depth), n), dtype=np.float64)
    f1_rf = np.zeros((len(max_features), len(depth), n), dtype=np.float64)
    ...
    for _ in range(2):
        for train_index, test_index in kf.split(df):
            ...
            for feature_number, j in zip(max_features, range(len(max_features))):
                for depth_number, k in zip(depth, range(len(depth))):
                    rf = RandomForestClassifier(n_estimators=20, criterion='entropy', max_depth=depth_number,
    ...
    plt.xticks(np.arange(len(depth)), labels=depth)
    plt.yticks(np.arange(len(max_features)), labels=max_features)
    ...
    fig.savefig('NRF_depth_maxFeatures_acc_messidor.png')
    ...
    for k in range(len(depth)):
        for j in range(len(max_features)):
            text = ax.text(k, j, u'{:.2f}\n\u00B1\n{:.2f}'.format(f1_nrf_mean[j, k], f1_nrf_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_depth_maxFeatures_f1_messidor.png')
    ...
    for k in range(len(depth)):
        for j in range(len(max_features)):
            text = ax.text(k, j, u'{:.2f}\n\u00B1\n{:.2f}'.format(accuracy_nrfdw_mean[j, k], accuracy_nrfdw_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_DW_depth_maxFeatures_acc_messidor.png')
    ...
    for k in range(len(depth)):
        for j in range(len(max_features)):
            text = ax.text(k, j, u'{:.2f}\n\u00B1\n{:.2f}'.format(f1_nrfdw_mean[j, k], f1_nrfdw_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_DW_depth_maxFeatures_f1_messidor.png')
    ...
    for k in range(len(depth)):
        for j in range(len(max_features)):
            text = ax.text(k, j, u'{:.2f}\n\u00B1\n{:.2f}'.format(accuracy_nrfeldw_mean[j, k], accuracy_nrfeldw_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_EL_DW_depth_maxFeatures_acc_messidor.png')
    ...
    for k in range(len(depth)):
        for j in range(len(max_features)):
            text = ax.text(k, j, u'{:.2f}\n\u00B1\n{:.2f}'.format(f1_nrfeldw_mean[j, k], f1_nrfeldw_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_EL_DW_depth_maxFeatures_f1_messidor.png')
    ...
    for k in range(len(depth)):
        for j in range(len(max_features)):
            text = ax.text(k, j,
                           ...
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_EL_DW_identity_depth_maxFeatures_acc_messidor.png')
    ...
    for k in range(len(depth)):
        for j in range(len(max_features)):
            text = ax.text(k, j, u'{:.2f}\n\u00B1\n{:.2f}'.format(f1_nrfeldw_ultra_mean[j, k], f1_nrfeldw_ultra_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('NRF_EL_DW_identity_depth_maxFeatures_f1_messidor.png')
        ...
        for j in range(len(max_features)):
            text = ax.text(k, j,
                           u'{:.2f}\n\u00B1\n{:.2f}'.format(accuracy_rf_mean[j, k],
                                                            accuracy_rf_std[j, k]),
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('RF_depth_maxFeatures_acc_messidor.png')
    ...
    for k in range(len(depth)):
        for j in range(len(max_features)):
            text = ax.text(k, j,
                           ...
                           ha="center", va="center", color="red", fontsize=7)
    fig.savefig('RF_depth_maxFeatures_f1_messidor.png')
