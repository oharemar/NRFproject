        ...
        for X,y in datasets:
            ...
            for beta in betas:
                for model in names:
                    ...
                    ax = plt.subplot(len(betas), len(names), i)
                    ax.set_xticks(())
                    ax.set_yticks(())
                    try:
                        if model == 'Input data':
                            ...
                            mod = NeuralRandomForest(rf, 'NRF_analyticWeights_adam', X_train, y_train,
                                                       ...
                                                       gamma_output=1, gamma=[beta, beta])  # zde změna, gamma_output je 1
                            ...
                            mod.compile(loss='categorical_crossentropy',
                                        optimizer=sgd,  # 'sgd',
                                        metrics=['accuracy'])
                            mod.fit(X_train, y_train_keras, epochs=beta)
                    except:
                        if i in [1, 2, 3, 4, 5, 6]:
                            ax.set_title(model)
                        ax.scatter(X_train[:, 0], X_train[:, 1], c=y_train, cmap=cm_bright, edgecolors='k')
            ...
            i = 1
            plt.tight_layout()
            fig.savefig('betas{}_samples{}'.format(dataset,sample))
            dataset += 1
