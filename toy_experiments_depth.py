...
if __name__ == '__main__':
    ...
    for sample in samples:
        ...
        names = ['Input data','Random Forest', 'NRF_DW', 'NRF_EL_DW', 'NRF_EL_DW_id', 'NRF']
        ...
        for X,y in datasets:
            ...
            for depth in depths:
                rf = RandomForestClassifier(n_estimators=10, max_depth=depth, random_state=1)
                rf.fit(X_train, y_train)
                for model in names:
                    ax = plt.subplot(len(depths), len(names), i)
                    ax.set_xticks(())
                    ax.set_yticks(())
                    try:
                        if model == 'Input data':
                            ax.set_ylabel('Depth = {}'.format(depth))
                            if i == 1:
                                ax.set_title(model)
                            ...
                            mod.compile(loss='categorical_crossentropy',
                                        optimizer=sgd,  # 'sgd',
                                        metrics=['accuracy'])
                            mod.fit(X_train, y_train_keras, epochs=depth)
                    except:
                        if i in [1, 2, 3, 4, 5, 6]:
                            ax.set_title(model)
                        ax.scatter(X_train[:, 0], X_train[:, 1], c=y_train, cmap=cm_bright, edgecolors='k')
            ...
            i = 1
            plt.tight_layout()
            fig.savefig('depths_{}_samples{}'.format(dataset,sample))
            dataset += 1
